...
deleteOldContents(){
    ...
    echo "Successfully deleted old bin/ndm contents"
}
...
moveCompiled(){
    ...
    mkdir -p "${MAIN_GOPATH[*]}"/bin/ndm/

    # Copy our OS/Arch to ${MAIN_GOPATH}/bin/ directory
    DEV_PLATFORM="./bin/ndm/$(go env GOOS)_$(go env GOARCH)"
    DEV_PLATFORM_OUTPUT=$(find "${DEV_PLATFORM}" -mindepth 1 -maxdepth 1 -type f)
    for F in ${DEV_PLATFORM_OUTPUT}; do
        cp "${F}" bin/ndm/
        cp "${F}" "${MAIN_GOPATH[*]}"/bin/ndm/
    done

    echo "Moved all the compiled things successfully to :${MAIN_GOPATH[*]}/bin/ndm"
}
...
build(){
    ...
    do
        ...
        do
            UNDERSCORE="_"
            output_name="bin/ndm/${GOOS}${UNDERSCORE}${GOARCH}/$CTLNAME"
...
# Move all the compiled things to the $GOPATH/bin/ndm
moveCompiled
