	...
	for _, device := range msg.Devices {
		...
		if pe.Controller.FeatureGates.IsEnabled(features.GPTBasedUUID) {
			...
			if !ok {
				...
				if err = d.AddPartition(0); err != nil {
...
func generateUUID(bd blockdevice.BlockDevice) (string, bool) {
	if bd.DeviceAttributes.DeviceType == libudevwrapper.UDEV_PARTITION {
		...
		return libudevwrapper.NDMBlockDevicePrefix + util.Hash(bd.PartitionInfo.PartitionEntryUUID), true
	}
	if len(bd.DeviceAttributes.WWN) > 0 {
		klog.Infof("device(%s) has a WWN, using WWN:%s", bd.DevPath, bd.DeviceAttributes.WWN)
		return libudevwrapper.NDMBlockDevicePrefix + util.Hash(bd.DeviceAttributes.WWN), true
	}
	if len(bd.PartitionInfo.PartitionTableUUID) > 0 {
		klog.Infof("device(%s) has a partition table, using parttable UUID:%s", bd.DevPath, bd.PartitionInfo.PartitionTableUUID)
		return libudevwrapper.NDMBlockDevicePrefix + util.Hash(bd.PartitionInfo.PartitionTableUUID), true
	}
	if len(bd.FSInfo.FileSystemUUID) > 0 {
		...
		return libudevwrapper.NDMBlockDevicePrefix + util.Hash(bd.FSInfo.FileSystemUUID), true
	}
	return "", false
}
