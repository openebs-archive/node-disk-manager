...
func (pe *ProbeEvent) addDiskEvent(msg controller.EventMessage) {
	...
	for _, diskDetails := range msg.Devices {
		...
		if oldDr != nil && len(diskDetails.PartitionData) != 0 {
			 ...
			 * For devices like LUN there will be device CR not a
			 * disk CR. Many device CRs may be backed by a one
			 * physical Disk incase disk is partitioned, where each
			 * partition is an indepedent device itself.
